  ...
  head: () => ({
    meta: [
      { title: "Strategy · The Dispute Playbook" },
      {
        name: "description",
        content:
          "The Phase Map, the 5-Round Strategy, and the Account Router — the strategic backbone of the Dispute Playbook.",
      },
    ],
  }),
...
const ROUNDS = [
  {
    ...
    n: "R1",
    t: "Open the dispute",
    b: "First contact. Identify the inaccurate item, request investigation. The clock starts.",
    letters: ["L11", "L15A"] as const,
  },
  {
    id: "round-2",
    n: "R2",
    t: "Cite the law",
    b: "MOV demand. Force the bureau or furnisher to describe how they actually verified.",
    letters: ["L12"] as const,
  },
  {
    id: "round-3",
    n: "R3",
    t: "Direct demand",
    b: "Stronger language, escalating tone, explicit consequences referenced.",
    letters: ["L13"] as const,
  },
  {
    id: "round-4",
    n: "R4",
    t: "Violation notice",
    b: "Document the violations on the record. This becomes Phase 6 evidence.",
    letters: ["L14"] as const,
  },
  {
    id: "round-5",
    n: "R5",
    t: "Final notice",
    b: "Last in-system step before regulator escalation.",
    letters: ["L19"] as const,
  },
];
      ...
      <div className="max-w-3xl space-y-24">
        <section id="phase-map" className="space-y-8 scroll-mt-24">
          <EditorialHeader
            ...
            title={
              <>
                The Phase <em className="font-editorial text-accent-grad">Map</em>.
              </>
            }
          ...
          <div className="space-y-3">
            {PHASES.map((p) => (
              <div
                key={p.id}
                className="rounded-xl border-2 border-border bg-card p-5 transition hover:border-[color:var(--brand-gold)]"
              >
        ...
        <section id="round-strategy" className="space-y-8 scroll-mt-24">
          <EditorialHeader
            ...
            title={
              <>
                The 5-Round <em className="font-editorial text-accent-grad">Strategy</em>.
              </>
            }
          ...
          <div className="space-y-4">
            {ROUNDS.map((r) => (
              <div
                key={r.id}
                id={r.id}
                className="scroll-mt-24 rounded-xl border-2 border-border bg-card p-5 transition hover:border-[color:var(--brand-magenta)]"
              >
                <div className="flex items-baseline gap-3">
                  <span className="font-mono text-xs font-bold text-[color:var(--brand-magenta-deep)]">
                    {r.n}
                  </span>
                  <h3 className="font-display text-xl font-bold text-[color:var(--brand-navy-deep)]">
                    {r.t}
                  </h3>
                </div>
                ...
                <div className="mt-3 flex flex-wrap gap-2">
                  {r.letters.map((l) => (
                    <LetterChip key={l} id={l} variant="pill" />
                  ))}
                </div>
              </div>
            ))}
        ...
        <section id="account-router" className="space-y-10 scroll-mt-24">
          <EditorialHeader
            ...
            title={
              <>
                The Account <em className="font-editorial text-accent-grad">Router</em>.
              </>
            }
          ...
          >
            <div className="flex items-center gap-3">
              ...
              <p className="eyebrow !mb-0 text-[color:var(--brand-navy-deep)]">
            ...
            <ol className="mt-5 space-y-3 text-base leading-relaxed text-foreground/90 md:text-lg">
              <li className="flex items-start gap-3">
                <span
                  className="font-display mt-0.5 inline-flex h-7 w-7 shrink-0 items-center justify-center rounded-full text-sm font-bold text-[color:var(--brand-cream)]"
                  style={{ background: "var(--brand-navy-deep)" }}
                >
                  1
                </span>
                <span>
                  Take ONE account from your credit report at a time. Do not batch — each account
              ...
              <li className="flex items-start gap-3">
                ...
                <span>
                  Read the four routes below in order. Stop at the FIRST route whose &ldquo;Use this
                  route when&rdquo; statement matches your account. Routes are listed in priority
                  order on purpose.
                </span>
              </li>
              <li className="flex items-start gap-3">
                ...
                <span>
                  Send the letters in that route in the exact numerical order shown. Do not skip
                  ahead — the legal weight of each later letter depends on the paper trail of the
                  earlier ones.
                </span>
              </li>
              <li className="flex items-start gap-3">
                ...
                <span>
                  If an account fits more than one route, run them in the order shown (Identity
                  always first if any personal-info issue exists, otherwise the lowest-numbered
                  matching route).
                </span>
              </li>
            </ol>
            <p className="mt-5 flex items-start gap-2 rounded-lg border border-dashed border-[color:var(--brand-gold)] bg-[color:color-mix(in_oklab,var(--brand-gold)_8%,var(--card))] p-3 text-sm text-foreground/85">
              ...
              <span>
                <strong className="font-bold">Identity route runs first, always.</strong> If your
                file has any wrong personal information, clean it before any other route — otherwise
                furnishers will reject every later dispute on identity mismatch.
              </span>
            </p>
          </div>
            ...
            const ROUTES = [
              {
                ...
                why: "Furnishers and bureaus use personal data to match you to accounts. If their copy of YOU is wrong, they will reject every dispute that follows on the grounds that they 'cannot verify the consumer.' Cleaning identity is non-negotiable foundation work.",
                phaseLabel: "Phase 3 · Clean Identity",
                phaseId: "clean-identity" as const,
                sequence: ["L05", "L06", "L07", "L08", "L09", "L10"] as const,
                expect:
                  "30–60 days for full cleanup. Most personal-info corrections are quick because the data is easy to verify.",
              },
              {
                key: "validation",
                priority: "Route 2 of 4 — Use only inside the 30-day window",
                priorityNote:
                  "Strict deadline. Miss day 30 and this route closes; switch to Route 3.",
                Icon: PhoneCall,
                colorVar: "--phase-2",
                ...
                why: "FDCPA §1692g gives you a one-time right to demand the collector prove the debt before they can keep collecting OR keep reporting. Roughly 30–40% of collectors never respond and the tradeline must come off. This is the cheapest, fastest route — but only if you are inside the window.",
              ...
              },
              {
                ...
                priorityNote:
                  "Use this for any inaccurate item you can name a specific factual error on.",
                Icon: FileWarning,
                colorVar: "--phase-4",
                ...
                why: "FCRA §1681i forces the bureaus to investigate within 30 days. The five-round letter sequence escalates the legal weight of each follow-up: accuracy → procedure exposure → intent to file → final notice → CFPB. Most disputes are resolved by Round 2 or 3.",
                phaseLabel: "Phase 4 · Dispute Bureaus",
                phaseId: "dispute-bureaus" as const,
                sequence: ["L11", "L12", "L13", "L14"] as const,
                expect:
                  "90–150 days for the full 4-round cycle, with 30-day pauses between rounds.",
              },
              {
                key: "furnisher",
                priority: "Route 4 of 4 — When the bureaus stall",
                priorityNote:
                  "Use after Route 3 returns 'verified' on data you know is wrong, OR run in parallel.",
                Icon: Building2,
                colorVar: "--phase-5",
                ...
                why: "Bureaus often rubber-stamp 'verified' by sending a 2-digit eOSCAR code to the furnisher who answers 'yes.' Going direct under §1681s-2(b) forces a real investigation by the furnisher AND opens an independent statutory-damages path the bureau-only route does not.",
                    ...
                    >
                      ...
                      <div className="mt-6 flex flex-wrap items-center justify-between gap-4 rounded-xl border border-dashed border-border/70 bg-card/60 p-4">
                        <p className="flex items-start gap-2 text-sm text-foreground/85">
                          ...
                          />
                          <span>
                            <strong className="font-bold text-[color:var(--brand-ink)]">
                              Expect:{" "}
                            </strong>
                            {r.expect}
                          </span>
                      ...
                      <div className="mt-4 flex flex-wrap gap-2">
                        {r.sequence.map((id) => (
                          <LetterChip key={id} id={id} variant="pill" />
                        ))}
                      </div>
                    </article>
                  );
        ...
        <section
          id="round-tracker"
          className="space-y-6 scroll-mt-24 rounded-2xl border border-border bg-[color:color-mix(in_oklab,var(--brand-navy)_92%,var(--brand-violet))] p-8 text-[color:var(--brand-cream)]"
        >
          <p className="eyebrow text-[color:var(--brand-gold)]">Reference · Round Tracker</p>
          <h2 className="font-display text-3xl font-bold">
            Track every <em className="font-editorial text-[color:var(--brand-gold)]">round</em>.
          </h2>
          <p className="font-editorial text-lg opacity-95">
            Date sent. Date received. Outcome. Next move. Log every back-and-forth on the Round
            Tracker. The paper trail is the case if{" "}
            <Ref to="phase-escalate" className="!text-[color:var(--brand-gold)]">
              Phase 6
            </Ref>{" "}
            escalation becomes necessary.
          </p>
          <p className="text-sm opacity-80">
            An interactive Round Tracker is on the roadmap — for now, print the kit's tracker page
