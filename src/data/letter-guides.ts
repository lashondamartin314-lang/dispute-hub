...
export const LETTER_GUIDES: Record<LetterId, LetterGuide> = {
  ...
  L01: {
    ...
    nextStep: [
      {
        condition: "No response within 30 days of their signed receipt",
        letterId: "L02",
        action: "Send L02 — Validation Follow-Up · No Response.",
      },
      {
        condition:
          "They sent paper that is NOT real validation (just a printout, statement, or screenshot)",
        letterId: "L03",
        action: "Send L03 — Dispute of Inadequate Validation.",
      },
      {
        condition:
          "Their response shows the data they are reporting is wrong (wrong balance, wrong dates, wrong owner)",
        letterId: "L04",
        action: "Send L04 — Validation Bridge · Errors Identified, then move into Phase 4.",
      },
      {
        condition: "They provide complete, accurate validation and the debt is genuinely yours",
        action:
          "Stop here. Decide whether to pay, settle in writing (get pay-for-delete in writing first — see L18), or wait it out.",
      },
    ],
  ...
  L02: {
    recipient: "The same collection agency you sent L01 to — no one else.",
    findAddress:
    ...
    nextStep: [
      {
        condition: "Tradeline disappears from all three bureaus within ~30 days",
        action: "Document the deletion (save before/after reports). Move on to other accounts.",
      },
      {
        condition: "They send late paperwork that is still inadequate",
        letterId: "L03",
        action: "Send L03 — Dispute of Inadequate Validation.",
      },
      {
        condition: "Still total silence after another 30 days",
        letterId: "L11",
        action: "Move the item into Phase 4 — Bureau Dispute (L11).",
      },
    ],
  ...
  L03: {
    ...
    nextStep: [
      {
        condition: "They go silent or send another inadequate response",
        letterId: "L11",
        action:
          "Move to Phase 4 — Bureau Dispute (L11) treating the account as 'unverifiable, still reporting'.",
      },
      {
        condition: "They send actual valid documentation",
        action:
          "Validation has been satisfied. Decide whether to pay/settle, or pivot to accuracy disputes if anything in their paperwork is wrong.",
      },
    ],
    legalBasis:
      "FDCPA §1692g(b) — 'verification' must include identification of the original creditor and the amount of the debt.",
  },
  ...
  L04: {
    ...
    nextStep: [
      {
        condition: "Bureau confirms the deletion / correction",
        action: "Save the result-letter PDF for your records. Move on.",
      },
      {
        condition: "Bureau says 'verified' but the error remains",
        letterId: "L12",
        action: "Escalate with L12 — Procedure Exposure / MOV Demand.",
      },
    ],
  ...
  L05: {
    ...
    nextStep: [
      {
        condition: "Creditor updates and bureaus follow",
        action: "Confirm on next month's reports, then move on.",
      },
      {
        condition: "Creditor refuses or ignores",
        letterId: "L06",
        action: "Go around them with L06 — Bureau Personal Information Dispute.",
      },
    ],
  ...
  L06: {
    ...
    nextStep: [
      {
        condition: "Bureau makes the correction",
        action:
          "Save the result letter. Pull a fresh report in 30 days to confirm the change held.",
      },
      {
        condition: "Bureau replies 'verified' without explanation",
        letterId: "L07",
        action: "Send L07 — Failure to Investigate follow-up.",
      },
      {
        condition: "Bureau ignores or doesn't respond at all in 30+ days",
        letterId: "L07",
        action: "Send L07 to put the missed deadline on the record.",
      },
    ],
  ...
  L07: {
    ...
    whenToSend:
      "Day 31 after your L06 was delivered, or as soon as you receive a 'verified' reply with no actual investigation detail.",
    prerequisites: [
      "L06 was delivered.",
      "30 days passed OR a non-substantive 'verified' reply was received.",
    ],
    mustInclude: [
    ...
    doNot: [
      "Do NOT re-list every single item — this letter is about THEIR failure to investigate, not the underlying data.",
    ],
    ...
    nextStep: [
      ...
      {
        condition: "Still 'verified' with no detail",
        letterId: "L09",
        action: "Escalate with L09 — Method of Verification demand.",
      },
    ],
  ...
  L08: {
    recipient:
      "All three bureaus. If theft is suspected, ALSO file an FTC IdentityTheft.gov report and attach it.",
    findAddress: "Bureau dispute P.O. Boxes (template-included).",
    whenToSend:
      "Immediately upon discovering accounts, addresses, or inquiries you do not recognize — or another person's data merged into your file.",
    prerequisites: [
    ...
    expect:
      "With an FTC report attached, bureaus must block the disputed information within 4 business days under §1681c-2.",
    nextStep: [
      {
        condition: "Items blocked",
        action: "Maintain the freeze, monitor monthly for re-insertion.",
      },
      {
        condition: "Bureau refuses to block despite FTC report",
        letterId: "L13",
        action: "Escalate immediately — this is a §1681c-2 violation. Use L13 framing.",
      },
    ],
  ...
  L09: {
    ...
    prerequisites: [
      "A prior dispute (L06 or L07) was answered with 'verified' but no description of how.",
    ],
    mustInclude: [
    ...
    mustDo: [
      "Certified Mail · Return Receipt.",
      "Be patient — bureaus have 15 days to provide MOV after request.",
    ],
    doNot: ["Do NOT confuse this with a re-dispute. This is a separate, named statutory request."],
    expect:
      "Either a vague reply (which becomes Phase 6 evidence), or a real description that often itself reveals the failure.",
    nextStep: [
      {
        condition: "MOV reveals the bureau just re-asked the furnisher who said 'yes'",
        letterId: "L13",
        action: "Send L13 — Notice of Intent to File Complaint (CFPB).",
      },
      {
        condition: "MOV reveals an actual investigation but the data is still wrong",
        letterId: "L15A",
        action: "Move to Phase 5 — Direct Furnisher Dispute (L15A or L15B).",
      },
    ],
  ...
  L10: {
    ...
    whenToSend:
      "Once when establishing your file's foundation — and any time you see your SSN or DOB displayed wrong on a report.",
    ...
    mustDo: [
      "Certified Mail · Return Receipt.",
      "Use last 4 only when typed in the body of the letter.",
    ],
  ...
  L11: {
    recipient:
      "The three credit bureaus — Equifax, Experian, TransUnion. One envelope each, per inaccurate item.",
    findAddress:
      "Bureau dispute P.O. Boxes (template-included). Never the online portal for round 1.",
    whenToSend:
    ...
    nextStep: [
      {
        condition: "Deleted or updated to correct",
        action: "Save the results letter. Move on to the next item.",
      },
      {
        condition: "Verified but still wrong",
        letterId: "L12",
        action: "Round 2 — send L12 (MOV Demand) within 15 days of their reply.",
      },
      {
        condition: "No response within 35 days of delivery",
        letterId: "L13",
        action:
          "Skip to Round 3 — L13 (intent to complain). The missed deadline is itself a violation.",
      },
    ],
  ...
  L12: {
    ...
    expect:
      "Bureau has 15 days to provide MOV. Many do not, or send a vague one — both outcomes strengthen Phase 6.",
    nextStep: [
      {
        condition: "MOV not provided OR vague",
        letterId: "L13",
        action: "Send L13 — Notice of Intent to File Complaint.",
      },
      {
        condition: "MOV reveals furnisher just rubber-stamped 'yes'",
        letterId: "L15A",
        action: "Open Phase 5 with L15A or L15B (direct furnisher dispute).",
      },
    ],
    legalBasis: "FCRA §1681i(a)(7) — Description of reinvestigation procedure.",
  },
  ...
  L13: {
    recipient:
      "The bureau(s) still resisting — and a copy of the letter goes into your CFPB complaint draft.",
    ...
    expect:
      "Roughly 50% of items are deleted within the 15-day cure window. The rest become L14 / Phase 6.",
    nextStep: [
      { condition: "Deleted", action: "Done. Save the result letter." },
      {
        condition: "Still no movement after 15 days",
        letterId: "L14",
        action: "Send L14 — Final Violation Notice and prepare the CFPB complaint.",
      },
    ],
  ...
  L14: {
    recipient:
      "The bureau, with full violation log attached. This letter doubles as your Phase 6 exhibit.",
    findAddress: "Bureau dispute P.O. Box.",
    ...
    doNot: [
      "Do NOT continue past Round 4 in-system. After L14 the right move is the regulator, not Round 5 in the same channel.",
    ],
    expect:
      "Final in-system response within 30 days; many items delete just before CFPB submission.",
    nextStep: [
      { condition: "Deleted", action: "Document everything. You're done with this item." },
      {
        condition: "Still no correction",
        action: "Open Phase 6 — file the CFPB complaint with your assembled evidence.",
      },
    ],
  ...
  L15A: {
    recipient:
      "The COLLECTION AGENCY (third-party debt buyer or contingency collector) reporting the tradeline.",
    findAddress:
      "On your credit report under the disputed account: 'Furnisher contact' or 'Creditor address'. If absent, use the address from any letter they have ever sent you.",
    whenToSend:
      "After Phase 4 either failed (verified-but-wrong) OR you want to attack the data at its source in parallel.",
    prerequisites: [
    ...
    expect:
      "Furnisher must investigate within 30 days and report results to the bureaus. Many quietly delete rather than respond.",
    nextStep: [
      { condition: "Deleted / corrected", action: "Confirm on next bureau pull. Done." },
      {
        condition: "Furnisher refuses or stalls",
        letterId: "L16",
        action: "Use L16 — Inconsistency Attack — to widen the front.",
      },
    ],
  ...
  L15B: {
    recipient:
      "The ORIGINAL CREDITOR (the bank / lender that issued the account) — used when they are still the furnisher of record, not a collector.",
    findAddress:
      "Creditor's 'Notice Address' or 'Credit Bureau Disputes' department on their website. If unsure, ask their customer service in writing for the correct dispute mailing address.",
    ...
    nextStep: [
      { condition: "Corrected / deleted", action: "Done." },
      {
        ...
        letterId: "L13",
        action: "Loop back into Phase 4 with renewed L13 and add the §1681s-2 violation.",
      },
    ],
  ...
  L15C: {
    ...
    findAddress: "Bureau dispute P.O. Boxes.",
    whenToSend:
      "Same day as L15A/B so the furnisher and the bureau investigate concurrently and cannot deflect to each other.",
    ...
    mustDo: [
      "Certified Mail · Return Receipt.",
      "Mail all four envelopes (furnisher + 3 bureaus) the same day.",
    ],
    doNot: [
      "Do NOT skip this step thinking 'the furnisher will tell them' — they often don't, and the bureau then claims it never knew.",
    ],
    expect: "Both ends investigate; results converge in 30 days.",
    nextStep: [
      { condition: "Deleted / corrected", action: "Done." },
      {
        condition: "Both verify the same wrong data",
        letterId: "L16",
        action: "Send L16 — Inconsistency Attack.",
      },
    ],
  ...
  L16: {
    ...
    findAddress: "Bureau dispute P.O. Boxes.",
    whenToSend:
      "When the same account is reported with materially different data across bureaus (different balance, different open date, different status).",
    prerequisites: [
    ...
    doNot: [
      "Do NOT send the same chart of inconsistencies for unrelated items — keep one issue per envelope.",
    ],
    expect:
      "Bureaus often delete rather than reconcile, because reconciliation requires re-contacting the furnisher.",
    nextStep: [
      {
        condition: "Deleted on at least two bureaus",
        action:
          "Use the deletions to leverage the third — re-dispute citing the deletions as evidence.",
      },
      {
        condition: "All three verify identically (rare)",
        letterId: "L17",
        action: "If charge-off, escalate with L17 specialty letter.",
      },
    ],
  ...
  L17: {
    ...
    whenToSend:
      "When a charge-off account shows continuing balance updates, status flips, re-aged Date of First Delinquency, or post-charge-off interest.",
    prerequisites: [
    ...
    doNot: [
      "Do NOT pay anything on a re-aged charge-off without first getting deletion in writing — payment can re-start the clock.",
    ],
    expect:
      "Most furnishers either correct or delete; few want to defend a re-aging pattern in writing.",
    nextStep: [
      { condition: "Deleted / corrected", action: "Done. Confirm DoFD held on next pull." },
      {
        condition: "Refused",
        action: "Add to Phase 6 packet — re-aging is a CFPB-priority violation.",
      },
    ],
  ...
  L18: {
    recipient:
      "The current owner of the debt (collector or original creditor) — only after validation has been satisfied or rejected, and only if you've decided you'd pay to make it disappear.",
    findAddress: "Same address you have been using for the account.",
    whenToSend:
      "Late in the process, after L01–L03 have run their course, when removal — not vindication — is the goal.",
    prerequisites: [
    ...
    expect:
      "Acceptance, counter-offer, or refusal within 30 days. Many large collectors have written policies against pay-for-delete; smaller ones often accept.",
    nextStep: [
      {
        condition: "Accepted in writing",
        action: "Pay by cashier's check, then verify deletion on the next two monthly reports.",
      },
      {
        condition: "Counter-offered",
        action: "Negotiate in writing only. Revise L18 with new terms; re-mail certified.",
      },
      {
        condition: "Refused",
        action:
          "Return to L11 / L15A and continue the dispute campaign — you have not damaged your case.",
      },
    ],
    legalBasis:
      "Contract law — there is no FCRA right to pay-for-delete; the leverage is purely commercial.",
  },

  L19: {
    recipient:
      "The lender or company that pulled the unauthorized hard inquiry. (Inquiries you did authorize are NOT eligible.)",
    findAddress:
      "Listed on your credit report next to the inquiry. If only a code appears, call the bureau to obtain the full inquirer name and address (do not dispute via the call — get the address only).",
    whenToSend:
      "Within 2 years of the inquiry date — though faster is better; lenders purge old records.",
    prerequisites: [
    ...
    mustDo: [
      "Certified Mail · Return Receipt.",
      "Mirror to all three bureaus listing the same inquiry as 'unauthorized — disputing with inquirer'.",
    ],
    doNot: [
    ...
    expect:
      "Many removals within 30–45 days. Some require a follow-up; statutory damages of up to $1,000 per willful unauthorized pull are available if litigated.",
    nextStep: [
      { condition: "Inquirer removes the pull", action: "Confirm on the next bureau pull. Done." },
      {
        condition: "Inquirer refuses or claims permissible purpose without proof",
        action:
          "Add to Phase 6 packet — CFPB and State AG complaint. Consider FCRA counsel for a §1681b violation case.",
      },
    ],
